import { GameState, LightningUser, LightningBalance } from '@/types/lightningTypes';
...
export type GameAction =
  | { type: GameActionType.LOGIN_SUCCESS; userId: string; lightningUser: LightningUser; balance: LightningBalance }
  | { type: GameActionType.LOGOUT }
  | { type: GameActionType.BUY_GOLD; satoshisAmount: number; goldAmount: number }
...
export function lightningGameStateReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    case GameActionType.LOGIN_SUCCESS:
      return {
        ...
        lightningUser: action.lightningUser,
    ...
    case GameActionType.BUY_GOLD:
      return {
        ...
        balance: state.balance ? {
          ...state.balance,
          satoshis: state.balance.satoshis - action.satoshisAmount
        } : null,
      };
